...
/**
 * WebRTC 控制器，用于公网 WAN 环境下的实时视频通信。
 */
@RestController
@RequestMapping("/api/wan")
public class WebRtcWANController {

    private final KurentoClient kurento = KurentoClient.create("ws://121.36.205.85:8888/kurento");
    ...
    // sessionId -> 收集到但未发送给前端的 candidates
    private final ConcurrentHashMap<String, List<IceCandidate>> pendingCandidates = new ConcurrentHashMap<>();
...
//        // 添加之前缓存的 ICE candidate
        ...
        // 收集 ICE Candidate
        session.publisherEndpoint.addOnIceCandidateListener(event -> {
            IceCandidate candidate = event.getCandidate();
            // 存储到pending
            pendingCandidates.computeIfAbsent(sessionId, k -> new CopyOnWriteArrayList<>())
                    .add(candidate);
        });
    ...
    // 提供轮询接口
    @GetMapping("/candidate/poll")
    public List<Map<String, Object>> pollCandidates(@RequestParam String sessionId) {
        List<IceCandidate> candidates = pendingCandidates.getOrDefault(sessionId, List.of());
        List<Map<String, Object>> res = new ArrayList<>();
        for (IceCandidate c : candidates) {
            res.add(Map.of(
                    "candidate", c.getCandidate(),
                    "sdpMid", c.getSdpMid(),
                    "sdpMLineIndex", c.getSdpMLineIndex()
            ));
        }
        // 清空已取走的
    ...
    public Map<String, Object> subscriberOffer(@RequestBody Map<String, Object> payload) {
        ...
        // 收集 ICE Candidate
        session.subscriberEndpoint.addOnIceCandidateListener(event -> {
            IceCandidate candidate = event.getCandidate();
            pendingCandidates.computeIfAbsent(sessionId, k -> new CopyOnWriteArrayList<>())
                    .add(candidate);
        });
